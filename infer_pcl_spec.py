...
def load_model(checkpoint_path, network_config, device):
    model = ULUNAS_PCL_Spec(**network_config).to(device)
...
def infer_directory(cfg_infer):
    ...
    for wav_name in tqdm(noisy_wavs, desc="infer_pcl_spec"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-C", "--config", default="configs/cfg_infer_pcl_spec.yaml")
